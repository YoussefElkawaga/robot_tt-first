...
class ConversationRobot:
    ...
    def send_message_to_arduino(self, message):
        """Send a message to the Arduino using the serial.py script for automatic permission handling"""
        ...
        # Use our serial.py script to send the command with automatic permission handling
        try:
            import subprocess
            import sys
            
            # Get the path to serial.py (in same directory as this script)
            script_dir = os.path.dirname(os.path.abspath(__file__))
            serial_script = os.path.join(script_dir, "serial.py")
            
            # Check if the script exists
            if not os.path.exists(serial_script):
                print(f"⚠️ Could not find serial.py script at {serial_script}")
                # Fall back to direct method
                return self._direct_send_message_to_arduino(message)
            
            # Run the script as a subprocess
            cmd = [sys.executable, serial_script, message.strip()]
            ...
            if result.returncode == 0:
                if result.stdout:
                    print(f"Serial.py output: {result.stdout.strip()}")
                return True
            else:
                print(f"⚠️ Error running serial.py: {result.stderr.strip()}")
                # Fall back to direct method
                return self._direct_send_message_to_arduino(message)
                
        except Exception as e:
            print(f"⚠️ Error calling serial.py: {e}")
            # Fall back to direct method
            return self._direct_send_message_to_arduino(message)
