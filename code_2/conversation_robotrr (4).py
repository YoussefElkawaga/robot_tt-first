...
class ConversationRobot:
    ...
    def get_ai_response(self, user_input, emotion_data=None):
        ...
        try:
            ...
            - Be mindful of potential sensory sensitivities
            - Never suggest activities that could be overstimulating
            - Offer alternatives that accommodate different sensory preferences
            - Recognize signs of sensory overload in the child's communication
            - Suggest calming techniques when appropriate
            - Respect preferences for routine and predictability
            ...
            - Use visual language and concrete examples to explain concepts
            - Avoid abstract metaphors or figurative language
            - Provide clear, step-by-step instructions for tasks
            - Give positive reinforcement for communication attempts
            - Be explicit about social rules that may not be intuitive
            - Use direct language rather than hints or implications
            - Support both verbal and non-verbal communication styles
            ...
            ROBOT INTERACTION GUIDELINES:
            - Use the robot's movements thoughtfully to enhance communication
            - Keep gestures predictable and gentle
            - Use movements to reinforce positive emotions (happy gesture for praise)
            - Avoid rapid or unpredictable movements that might be startling
            - Use the robot's physical presence to create a sense of companionship
    ...
    def speak(self, text):
        ...
        try:
            ...
            # First, do a short happy animation to show excitement
            print("Starting with a happy animation before talking...")
            self.send_message_to_arduino("happy\n")
            # Wait for a short time to let the happy animation play
            time.sleep(1.5)
            
            # Then, set to idle pose for the actual talking
            print("Setting to idle pose for talking...")
                ...
                if self.listen_for_wake_word():
                    ...
                    if user_input:
                        ...
                        if ai_response:
                            ...
                            # First show excitement with a happy animation
                            print("Starting with a happy animation before talking...")
                            self.send_message_to_arduino("happy\n")
                            time.sleep(1.5)  # Allow happy animation to play
                            
                            # Set to idle pose for the actual talking
                            print("Setting to idle pose for talking...")
                            self.send_message_to_arduino("idle\n")
                            time.sleep(0.5)
                            
                            print(f"Sending talk command to ensure animation (duration: {talk_duration}ms)")
                            self.send_message_to_arduino(f"talk:{talk_duration}\n")
                        ...
                        if 'Raspberry Pi' in f.read():
                            ...
                            try:
                                ...
                                if 'dialout' not in groups_output:
                                    ...
                                    # Try to automatically fix permissions for multiple possible Arduino ports
                                    for port_pattern in ['/dev/ttyACM*', '/dev/ttyUSB*']:
                                        for port_path in glob.glob(port_pattern):
                                            try:
                                                subprocess.run(['sudo', 'chmod', '666', port_path], check=False)
                                                print(f"✅ Applied permissions fix for {port_path}")
                                            except:
                                                pass
                            except:
                                pass
            ...
            try:
                ...
                try:
                    ...
                    if "permission" in str(e).lower() or "access" in str(e).lower():
                        print("\n⚠️ Permission error detected. Attempting automatic fix...")
                        ...
                        try:
                            ...
                            # Try connecting again with fixed permissions
                            self.arduino_serial = serial.Serial(
                                port=self.arduino_port,
                                baudrate=self.arduino_baud_rate,
                                timeout=1,
                            ...
                            print(f"❌ Failed to fix permissions: {perm_e}")
                            # Try one more method for Raspberry Pi
                            if is_raspberry_pi:
                                try:
                                    print("Trying alternative permission fix method...")
                                    # Try a more aggressive permission fix
                                    subprocess.run(['sudo', 'chmod', 'a+rw', self.arduino_port], check=False)
                                    # Also try adding current user to dialout group
                                    import getpass
                                    current_user = getpass.getuser()
                                    subprocess.run(['sudo', 'usermod', '-a', '-G', 'dialout', current_user], check=False)
                                    print(f"✅ Added user {current_user} to dialout group")
                                    print("Please log out and log back in for group changes to take effect")
                                    print("For now, trying one more connection attempt...")
                                    time.sleep(1)
                                    # Try one more time
                                    self.arduino_serial = serial.Serial(
                                        port=self.arduino_port,
                                    ...
                                    print(f"❌ Alternative permission fix failed: {alt_e}")
                                    # If still failed, try alternative ports
                                    return self.try_alternative_ports()
                    else:
                        # Not a permission error, try alternative ports
                        print(f"❌ Serial exception (not permissions): {e}")
                        return self.try_alternative_ports()
                ...
                print(f"❌ Failed to connect to {self.arduino_port}: {e}")
    ...
    def send_message_to_arduino(self, message):
        """Send a message to the Arduino using the arduino_helper.py script for automatic permission handling"""
        ...
        # Use our arduino_helper.py script to send the command with automatic permission handling
        try:
            import subprocess
            import sys
            
            # Get the path to arduino_helper.py (in same directory as this script)
            script_dir = os.path.dirname(os.path.abspath(__file__))
            helper_script = os.path.join(script_dir, "arduino_helper.py")
            
            # Check if the script exists
            if not os.path.exists(helper_script):
                print(f"⚠️ Could not find arduino_helper.py script at {helper_script}")
                # Fall back to direct method
                return self._direct_send_message_to_arduino(message)
            
            # Run the script as a subprocess
            cmd = [sys.executable, helper_script, message.strip()]
            
            # Add port parameter if we have a specific one
            if hasattr(self, 'arduino_port') and self.arduino_port:
                cmd.append(self.arduino_port)
            ...
            if result.returncode == 0:
                if result.stdout:
                    print(f"Helper script output: {result.stdout.strip()}")
                return True
            else:
                print(f"⚠️ Error running arduino_helper.py: {result.stderr.strip()}")
                # Fall back to direct method
                return self._direct_send_message_to_arduino(message)
                
        except Exception as e:
            print(f"⚠️ Error calling arduino_helper.py: {e}")
            # Fall back to direct method
            return self._direct_send_message_to_arduino(message)
    
    def _direct_send_message_to_arduino(self, message):
        """Direct method to send message to Arduino (used as fallback)"""
        print("Using fallback direct Arduino connection method...")
        ...
        while retry_count < max_retries:
            try:
                ...
                print(f"Sending message to Arduino directly: {message.strip()}")
